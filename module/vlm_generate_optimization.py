"""Runtime helpers for Alpamayo VLM generation profiling and safe optimizations."""
...
def optimized_vlm_generate(
    ...
    *,
    ...
    Alpamayo's trajectory rollout requests returned generation logits, but the
    normal trajectory path only consumes sequences and KV cache. Disabling
    returned logits avoids storing a large per-token tensor without changing
    token sampling. The original generation config value is restored after the
    call so the patch is local and reversible.
    """
